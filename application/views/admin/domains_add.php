...
<div class="main-content">
	<div class="adddomain">
			<form action="<?php if (isset($domain['ID_domain']) == false) {echo site_url('/domains/add');} else {echo site_url('/domains/edit'), '/', $domain['ID_domain'];} ?>" method="post">
				<div class="form-group"><label><?php echo lang('lbl_date_time_start'); ?>:</label> <input id="dtdomain1" class="form-control" type="datetime" name="date_time_start" placeholder="(yyyy-MM-dd hh:mm:ss)" value="<?php if(isset($domain['date_time_start']) == TRUE) echo $domain['date_time_start'];	?>"></div>
				<div class="form-group"><label><?php echo lang('lbl_date_time_expires'); ?>:</label> <input id="dtdomain2" class="form-control" type="datetime" placeholder="(yyyy-MM-dd hh:mm:ss)" name="date_time_expires" value="<?php if(isset($domain['date_time_expires']) == TRUE) echo $domain['date_time_expires']; ?>"></div>
